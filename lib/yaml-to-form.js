...
class YamlToForm {
  ...
  generateUiSchema (schemaProps) {
    ...
    Object.keys(schemaProps).forEach(section => {
  ...
  generateConditionalSchema () {
    const schema = {}
    Object.keys(this.yaml.sections).forEach(section => {
      this.yaml.sections[section].widgets.forEach(widget => {
        if (widget.condition) {
